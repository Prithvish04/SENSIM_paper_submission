...
class PilotNet_app(): #1NCC per layer
    def __init__(self, simObj):
        ...
        self.sim.output_dir = "DNNs/PilotNet/mapping_experiment/outputs/outputs_map1/"
...
class PilotNet_app_map2(): # 3NCC-->2NCC-->1NCC per layer 
    def __init__(self, simObj):
        self.sim = simObj
        # Modify experiment specific parameters
        from DNNs.PilotNet.mapping_experiment.sensim_app.parameters import parameters
        self.sim.output_dir = "DNNs/PilotNet/mapping_experiment/outputs/outputs_map3/"
    ...
    def composeLayersCoresMap(self):
        ...
        self.layer_core_map =  { self.layers['I'] : ((self.sim.CI,[range(0,66), range(0,200), range(0,3)], [0.0, 0.0]),),
                            ...
                            # L1 : shape=[31,98,24] devide to 3 NCCs
                            self.layers['L1']:( (self.sim.coresList[0][0],[range(  0, 31), range(  0, 98), range(  0,  8)], [0.0, 0.0]), 
                                                (self.sim.coresList[0][1],[range(  0, 31), range(  0, 98), range(  8, 16)], [0.0, 0.0]), 
                                                (self.sim.coresList[0][2],[range(  0, 31), range(  0, 98), range( 16, 24)], [0.0, 0.0]),), 
                            
                            # L2 : shape=[14,47,36] devide to 2 NCCs
                            self.layers['L2']:( (self.sim.coresList[1][0],[range(  0, 14), range(  0, 47), range(  0, 18)], [0.0, 0.0]), 
                                                (self.sim.coresList[1][1],[range(  0, 14), range(  0, 47), range( 18, 36)], [0.0, 0.0]),),
                            # L3 : shape=[ 5,22,48]
                            self.layers['L3']:( (self.sim.coresList[2][0],[range(  0,  5), range(  0, 22), range(  0, 48)], [0.0, 0.0]), ),
                            # L4 : shape=[ 3,20,64]
                            self.layers['L4']:( (self.sim.coresList[3][0],[range(  0,  3), range(  0, 20), range(  0, 64)], [0.0, 0.0]), ), 
        ...
        return self.layer_core_map
    def composeBusSegmentsList(self):
        segI = interconnect(master_cores=[self.sim.CI],                 slave_cores=self.sim.coresList[0][0:3], name='segI')
        seg1 = interconnect(master_cores=self.sim.coresList[0][0:3],    slave_cores=self.sim.coresList[1][0:2], name='seg1')
        seg2 = interconnect(master_cores=self.sim.coresList[1][0:2],    slave_cores=self.sim.coresList[2][0:1], name='seg2')
        seg3 = interconnect(master_cores=self.sim.coresList[2][0:1],    slave_cores=self.sim.coresList[3][0:1], name='seg3')
        seg4 = interconnect(master_cores=self.sim.coresList[3][0:1],    slave_cores=self.sim.coresList[4][0:1], name='seg4')
        seg5 = interconnect(master_cores=self.sim.coresList[4][0:1],    slave_cores=self.sim.coresList[5][0:1], name='seg5')
        seg6 = interconnect(master_cores=self.sim.coresList[5][0:1],    slave_cores=self.sim.coresList[6][0:1], name='seg6')
...
class PilotNet_app_map3(): # 6NCC-->2NCC-->1NCC per layer 
    def __init__(self, simObj):
        self.sim = simObj
        # Modify experiment specific parameters
        from DNNs.PilotNet.mapping_experiment.sensim_app.parameters import parameters
        self.sim.output_dir = "DNNs/PilotNet/mapping_experiment/outputs/outputs_map2/"
    ...
    def composeBusSegmentsList(self):
        ...
        seg1 = interconnect(master_cores=self.sim.coresList[0][0:6],    slave_cores=self.sim.coresList[1][0:2], name='seg1')
        seg2 = interconnect(master_cores=self.sim.coresList[1][0:2],    slave_cores=self.sim.coresList[2][0:1], name='seg2')
        seg3 = interconnect(master_cores=self.sim.coresList[2][0:1],    slave_cores=self.sim.coresList[3][0:1], name='seg3')
        seg4 = interconnect(master_cores=self.sim.coresList[3][0:1],    slave_cores=self.sim.coresList[4][0:1], name='seg4')
        seg5 = interconnect(master_cores=self.sim.coresList[4][0:1],    slave_cores=self.sim.coresList[5][0:1], name='seg5')
