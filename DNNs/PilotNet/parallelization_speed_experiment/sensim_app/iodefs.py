...
class PilotNetIO():
    def __init__(self, simObj):
        self.sim = simObj
        self.fps = 0 # put FPS=0 for max push
        self.time_between_events = 10 # the number of cycles between events (put 0 for max push)
        self.sim.plot_outputs = False
    ...
    def fetchData(self): 
        if self.sim.CI.getQueue()['out_queue_occupancy'].value<self.sim.param.queue_depths[0] and self.last_sample_converted<self.num_samples:
            ...
            self.time_input_event = utils.frame2event(input_frame=self.input_images[self.last_sample_converted], time_between_events=self.time_between_events, shuffle_pixels=True, queue=self.sim.CI.getQueue(), input_layer_ind=utils.findInputLayerFromLayerMap(self.sim.layer_core_map).ID, delta=True, start_time=self.time_input_event, frame_buffer=self.input_frame_buffer, threshold=self.threshold, parameters=self.sim.param)
