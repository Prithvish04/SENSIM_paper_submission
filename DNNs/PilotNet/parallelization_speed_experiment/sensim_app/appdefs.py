...
class PilotNet_app():
    def __init__(self, simObj, NrPhysicalCore4Cores=1, time_step=10000):
        self.sim = simObj
        
        # define experiment specific parameters
        from DNNs.PilotNet.NoC_experiment.sensim_app.parameters import parameters
        self.sim.param = parameters()

        self.sim.param.NrPhysicalCore4Cores = NrPhysicalCore4Cores
        self.sim.param.NrPhysicalCore4Buses = 1

        self.sim.output_dir = "DNNs/PilotNet/parallelization_speed_experiment/outputs/n_thread"+str(NrPhysicalCore4Cores)+"_timestep"+str(time_step)+"/"
        os.makedirs(self.sim.output_dir, exist_ok=True) 
        
        #Timing
        self.sim.simulation_end_time = 1e9
        self.sim.param.evaluation_time_step= 1e6
        self.sim.param.snapshot_log_time_step = 1e6*1000
        self.sim.param.time_step= time_step
    ...
    def composeLayersCoresMap(self): # 6NCC --> 6NCC --> 6NCC --> 6NCC --> 1NCC per layer
        ...
        self.layer_core_map =  { self.layers['I'] : ((self.sim.CI,[range(0,66), range(0,200), range(0,3)], [0.0, 0.0]),),
                            ...
                            # L1 : shape=[31,98,24] devide to 6 NCCs
                            self.layers['L1']:( (self.sim.coresList[0][0],[range(  0, 31), range(  0, 49), range(  0,  8)], [0.0, 0.0]), 
                                                (self.sim.coresList[0][1],[range(  0, 31), range(  0, 49), range(  8, 16)], [0.0, 0.0]), 
                                                (self.sim.coresList[0][2],[range(  0, 31), range(  0, 49), range( 16, 24)], [0.0, 0.0]),
                                                (self.sim.coresList[0][3],[range(  0, 31), range( 49, 98), range(  0,  8)], [0.0, 0.0]), 
                                                (self.sim.coresList[0][4],[range(  0, 31), range( 49, 98), range(  8, 16)], [0.0, 0.0]), 
                                                (self.sim.coresList[0][5],[range(  0, 31), range( 49, 98), range( 16, 24)], [0.0, 0.0]),), 
                            
                            # L2 : shape=[14,47,36] devide to 6 NCCs
                            self.layers['L2']:( (self.sim.coresList[1][0],[range(  0, 14), range(  0, 47), range(  0, 6)], [0.0, 0.0]), 
                                                (self.sim.coresList[1][1],[range(  0, 14), range(  0, 47), range(  6, 12)], [0.0, 0.0]),
                                                (self.sim.coresList[1][2],[range(  0, 14), range(  0, 47), range( 12, 18)], [0.0, 0.0]),
                                                (self.sim.coresList[1][3],[range(  0, 14), range(  0, 47), range( 18, 24)], [0.0, 0.0]),
                                                (self.sim.coresList[1][4],[range(  0, 14), range(  0, 47), range( 24, 30)], [0.0, 0.0]),
                                                (self.sim.coresList[1][5],[range(  0, 14), range(  0, 47), range( 30, 36)], [0.0, 0.0]),),
                            # L3 : shape=[ 5,22,48] devide to 6 NCCs
                            self.layers['L3']:( (self.sim.coresList[2][0],[range(  0,  5), range(  0, 22), range(  0,  8)], [0.0, 0.0]), 
                                                (self.sim.coresList[2][1],[range(  0,  5), range(  0, 22), range(  8, 16)], [0.0, 0.0]),
                                                (self.sim.coresList[2][2],[range(  0,  5), range(  0, 22), range( 16, 24)], [0.0, 0.0]),
                                                (self.sim.coresList[2][3],[range(  0,  5), range(  0, 22), range( 24, 32)], [0.0, 0.0]),
                                                (self.sim.coresList[2][4],[range(  0,  5), range(  0, 22), range( 32, 40)], [0.0, 0.0]),
                                                (self.sim.coresList[2][5],[range(  0,  5), range(  0, 22), range( 40, 48)], [0.0, 0.0]),),
                            # L4 : shape=[ 3,20,64]  devide to 6 NCCs
                            self.layers['L4']:( (self.sim.coresList[3][0],[range(  0,  3), range(  0, 20), range(  0, 10)], [0.0, 0.0]),
                                                (self.sim.coresList[3][1],[range(  0,  3), range(  0, 20), range( 10, 20)], [0.0, 0.0]),
                                                (self.sim.coresList[3][2],[range(  0,  3), range(  0, 20), range( 20, 31)], [0.0, 0.0]),
                                                (self.sim.coresList[3][3],[range(  0,  3), range(  0, 20), range( 31, 42)], [0.0, 0.0]),
                                                (self.sim.coresList[3][4],[range(  0,  3), range(  0, 20), range( 42, 53)], [0.0, 0.0]),
                                                (self.sim.coresList[3][5],[range(  0,  3), range(  0, 20), range( 53, 64)], [0.0, 0.0]), ), 
                            # L5 : shape=[ 1,18,64]
    ...
    def composeBusSegmentsList(self):
        segI = interconnect(master_cores=[self.sim.CI],                 slave_cores=self.sim.coresList[0][0:6], name='segI')
        seg1 = interconnect(master_cores=self.sim.coresList[0][0:6],    slave_cores=self.sim.coresList[1][0:6], name='seg1')
        seg2 = interconnect(master_cores=self.sim.coresList[1][0:6],    slave_cores=self.sim.coresList[2][0:6], name='seg2')
        seg3 = interconnect(master_cores=self.sim.coresList[2][0:6],    slave_cores=self.sim.coresList[3][0:6], name='seg3')
        seg4 = interconnect(master_cores=self.sim.coresList[3][0:6],    slave_cores=self.sim.coresList[4][0:1], name='seg4')
