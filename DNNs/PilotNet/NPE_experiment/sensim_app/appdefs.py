...
class PilotNet_app():
    def __init__(self, simObj, N_NPE=1.0):
        self.sim = simObj
        # Modify experiment specific parameters
        from DNNs.PilotNet.NPE_experiment.sensim_app.parameters import parameters
        self.sim.output_dir = "DNNs/PilotNet/NPE_experiment/outputs/outputs_map2/"

        os.makedirs(self.sim.output_dir, exist_ok=True)
        self.sim.param = parameters()
        
        #Timing
        self.sim.simulation_end_time = 10000e6
        self.sim.param.evaluation_time_step= 1e6
        self.sim.param.snapshot_log_time_step = 1e6
        self.sim.param.time_step= 1e4

        #HW
        self.sim.param.N_NPE = N_NPE
